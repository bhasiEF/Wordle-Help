...
response = requests.get('https://raw.githubusercontent.com/bhasiEF/Wordle-Help/main/csw19.csv')
response.raise_for_status()
master_list = np.loadtxt(io.BytesIO(response.content),dtype=str)

for i in range (0,8):  
  response = requests.get(f'https://github.com/bhasiEF/Wordle-Help/blob/main/relations_csw19_{i}.npy?raw=true', allow_redirects=True)
  response.raise_for_status()
  if i == 0:
    relations = np.load(io.BytesIO(response.content))
  else:
    relations = np.hstack((relations,np.load(io.BytesIO(response.content))))

response = requests.get('https://github.com/bhasiEF/Wordle-Help/blob/main/seed_entropies_no_prior.npy?raw=true')
response.raise_for_status()
info = np.load(io.BytesIO(response.content))
